...
const { initCloudBySource } = require('./utils/env');
...
exports.main = async (event, context) => {
  ...
  try {
    console.log(`[appApi v2.1.0] [${requestId}] 收到请求 - operation:`, event.operation, 'dataEnv:', event.dataEnv || event.data?.dataEnv || 'test(默认)');
    ...
    console.log(`[appApi] [${requestId}] 环境初始化完成 - dataEnv: ${envDataEnv}, envId: ${envId}, source: ${source || 'v2'}`);
    ...
    if (!tokenValidation.valid) {
      return error(401, tokenValidation.error || "未登录");
    }
    ...
    if (operationMapping[operation]) {
      console.log(`[appApi] 接口映射: ${operation} -> ${operationMapping[operation]}`);
    ...
    if (normalizedOperation.startsWith('appLogin') || 
      ...
      return {
        code: 404,
        message: `未找到对应的模块: ${operation}`
      };
    }

    // 调用模块方法
    if (module && typeof module[methodName] === 'function') {
      console.log(`[appApi] [${requestId}] 调用模块方法: ${methodName}`);
      ...
      console.log(`[appApi] [${requestId}] 请求完成 - code: ${result?.code || 'N/A'}`);
      return result;
    } else {
      console.warn(`[appApi] [${requestId}] 未实现的方法: ${methodName}`);
      return {
        code: 404,
        message: `未实现的方法: ${methodName}`
      };
    }
  } catch (err) {
    console.error(`[appApi v2.1.0] [${requestId}] 错误:`, err.message, err.stack);
    return {
      code: 500,
