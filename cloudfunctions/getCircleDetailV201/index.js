...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 获取圈子信息（直接查库，避免依赖 commonRequestV201 导致 FUNCTION_NOT_FOUND）
    let result = null;
    try {
      const list = (await db.collection('circle').aggregate()
        .match({ _id: circleId })
        .end()).list;
      result = (list && list.length) ? list[0] : null;
    } catch (e) {
      console.warn('[getCircleDetail] 查 circle 集合异常:', e.message);
    }
    ...
    if (result.errMsg || (result.code != null && result.code !== 200)) {
      return {
        code: result.code || 400,
        message: result.message || result.errMsg || '获取圈子详情失败',
        openId
      };
    }
    ...
    return {
      code: 400,
      message: error && error.message ? error.message : '获取圈子详情失败'
    };
  }
}
