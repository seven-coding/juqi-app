...
// 生产/测试环境 ID（与 appApiV201/utils/env.js 一致，用于 dataEnv 切换）
...
exports.main = async (event, context) => {
  ...
  if (type == 1) {
    ...
    // 首屏仅返回列表，目标 1 秒内；未读角标由客户端单独请求 type=1
    let result = await getMessagesUser({ openId, limit, page });
    ...
    // 获取关注消息（可选 from：会话详情页按「某人的关注消息」筛选）
