...
function convertDynToAppFormat(dyn, currentOpenId) {
  ...
  return {
    ...
    chargeCount: dyn.chargeNums || dyn.likeNums || 0, // dyn 文档无独立 chargeNums，充电=点赞，等于 likeNums
...
async function GetDynList(event) {
  ...
  try {
    ...
    console.log(`[reqId=${reqId}][appGetDynList] 入参: type=${type}, limit=${limit}, circleId=${circleId || '-'}, topic=${topic || '-'}, dataEnv=${event.dataEnv || '-'}, envId=${event.envId || '-'}`);
    ...
    console.log(`[reqId=${reqId}][appGetDynList] 调用子云函数: name=${getFuncName('getDynsListV2')}, envId=${coreParams.envId || '-'}`);
    ...
    if (result.result.code !== 200) {
      console.log(`[reqId=${reqId}][appGetDynList] 错误: code=${result.result.code || 500}, message=${result.result.message || '获取动态列表失败'}`);
      return error(result.result.code || 500, result.result.message || '获取动态列表失败');
    }
    console.log(`[reqId=${reqId}][appGetDynList] 返回: code=200, listCount=${(result.result.dynList || []).length}`);
    ...
    console.error(`[reqId=${reqId}][appGetDynList] 错误:`, err.message, err.stack);
...
async function GetDynDetail(event) {
  ...
  try {
    ...
    if (!id) {
      console.log(`[reqId=${reqId}][appGetDynDetail] 错误: code=400, message=缺少动态ID`);
      return error(400, "缺少动态ID");
    }
    console.log(`[reqId=${reqId}][appGetDynDetail] 入参: id=${id}, dataEnv=${event.dataEnv || '-'}, envId=${event.envId || '-'}`);

    // 调用核心层（传入 envId 与列表一致，避免再次进入时查错库导致 404）
    const callData = {
      id: id,
      openId: openId,
      ownOpenId: openId,
    ...
    console.log(`[reqId=${reqId}][appGetDynDetail] 调用子云函数: name=${getFuncName('getDynDetail')}, id=${id}`);
    const result = await cloud.callFunction({
      name: getFuncName('getDynDetail'),
      data: callData
    });
    ...
    if (coreResult.error || (coreResult.code && coreResult.code !== 200)) {
      console.log(`[reqId=${reqId}][appGetDynDetail] 错误: code=${coreResult.code || 500}, message=${coreResult.message || '获取动态详情失败'}, dynId=${id}`);
      return error(coreResult.code || 500, coreResult.message || '获取动态详情失败');
    }
    console.log(`[reqId=${reqId}][appGetDynDetail] 返回: code=200, dynId=${id}`);
    ...
    console.error(`[reqId=${reqId}][appGetDynDetail] 错误:`, err.message, err.stack);
    return error(500, err.message || '服务器错误');
  }
}
...
async function PublishDyn(event) {
  ...
  try {
    ...
    // 传入 dataEnv，确保 publishDyn 写入与当前请求一致的数据环境（test/prod）
...
async function LikeDyn(event) {
  ...
  try {
    ...
    // 调用核心层（传 envId 供核心层按环境初始化 DB）
    const result = await cloud.callFunction({
      name: getFuncName('likeOrUnlikeV2'),
      data: {
        id: id,
        type: 1, // 1=点赞动态
        openId: openId,
        dataEnv: event.dataEnv || 'test',
        envId: event.envId || undefined,
        requestId: event.requestId || ''
      }
    });
    ...
    console.error(`[reqId=${reqId}][appLikeDyn] 错误:`, err.message, err.stack);
    return error(500, err.message || '服务器错误');
  }
}

/**
 * 充电动态（走点赞逻辑：与点赞共用 like 数组与 likeNums，type=1）
 * 核心层: likeOrUnlikeV2, type=1
 */
async function ChargeDyn(event) {
  ...
  try {
    ...
    // 充电走点赞逻辑：调用点赞接口 type=1（传 envId 供核心层按环境初始化 DB，与列表/详情同库）
    const dataEnv = event.dataEnv || 'test';
    const envId = event.envId || undefined;
    console.log(`[reqId=${reqId}][appChargeDyn] 调用 likeOrUnlikeV201 - id=${id}, dataEnv=${dataEnv}, envId=${envId || '-'}`);
    const result = await cloud.callFunction({
      name: getFuncName('likeOrUnlikeV2'),
      data: {
        id: id,
        type: 1, // 1=点赞动态（充电与点赞共用同一套逻辑）
        openId: openId,
        dataEnv: dataEnv,
        envId: envId,
        requestId: event.requestId || ''
    ...
    if (res.code !== 200) {
      // 已点赞（充电=点赞，视为已充电成功）
      if (res.code === 201 && (res.message || '').indexOf('点赞') !== -1) {
        return success({ isLiked: true, likeNums: res.likeNums, chargeCount: res.likeNums });
      }
      return error(res.code || 500, res.message || '充电失败');
    }

    return success({
      isLiked: res.isLiked,
      likeNums: res.likeNums,
      chargeCount: res.likeNums
    });
  } catch (err) {
    console.error(`[reqId=${reqId}][appChargeDyn] 错误:`, err.message, err.stack);
    return error(500, err.message || '服务器错误');
  }
}
...
async function DeleteDyn(event) {
  ...
  try {
    ...
    console.error(`[reqId=${reqId}][appDeleteDyn] 错误:`, err.message, err.stack);
    return error(500, err.message || '服务器错误');
  }
}
...
async function GetDynComment(event) {
  ...
  try {
    ...
    console.error(`[reqId=${reqId}][appGetDynComment] 错误:`, err.message, err.stack);
